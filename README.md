...
3. Launch android studio, click `File` -> `Open`, navigate to the repository, open the `pLuck/` folder, and then open `build-gradle.kts`
4. After android studio syncs with gradle, receive a copy of the API, and other info required for Cloudinary, and copy and paste them into the file `local.properties` in the `pLuck/` folder. Note: The `local.properties` file is only created after gradle syncs with android studio.
